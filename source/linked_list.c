...
LinkedList *LinkedList_create() {
    LinkedList *newList = calloc(1, sizeof(LinkedList)); /* zeroed allocation, just what we need */
    if (!newList) {
...
/* todo JJ perhaps confusing with get_index since it returns a node* */
...
/*  fix JJ i think there is a bug here, head is not null when function returns */
void LinkedList_dispose(LinkedList *list, void free_function(void *)) {
    LinkedList_clear(list, free_function);
    free(list);
}

/* free all payloads and nodes but dont touch the list */
void LinkedList_free_nodes_and_payloads(LinkedList *list) {
    if (list == NULL) return;
    while (list->size) {
        free(LinkedList_get_index(list, list->size - 1)->item);
        LinkedList_remove(list, LinkedList_get_index(list, list->size - 1));
    }
}